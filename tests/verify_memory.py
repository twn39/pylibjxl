...
async def stress_test(iterations=100, concurrency=10, threads=4):
    print(
        f"Starting stress test: {iterations} iterations, {concurrency} concurrent tasks, {threads} runner threads"
    )
        ...
        try:
            for i in range(iterations):
                ...
                for _ in range(concurrency):
                    tasks.append(
                        runner.encode_async(img, effort=1)
                    )  # Use effort=1 for speed
                ...
                if (i + 1) % 50 == 0:
                    ...
                    print(
                        f"Iteration {i + 1}: {current_mem:.2f} MB (Delta: {diff:+.2f} MB)"
                    )
    ...
    print(
        f"Final Memory: {end_mem:.2f} MB (Total Delta: {end_mem - start_mem:+.2f} MB)"
    )
